...
### 긴 반복 허가 선택지와 Claude Code 2.1.28x 승인 화면 · 0.2.27

사용자 보고: `1. Yes / 2. Yes… / 3. No`에서 2번 문구가 길면 가끔 승인하지 않고 대기한다. 긴 선택지의 이어지는 줄은 문자 모양이 아니라 터미널이 그린 위치로 판별한다. 선택지는 이전 라벨보다 왼쪽인 번호 열에서 시작하고, 긴 라벨은 해당 라벨의 시작 열로 줄바꿈된다. 라벨 열까지 들여쓴 줄은 `>`(리다이렉트·화살표 함수)나 `2.`로 시작해도 앞 선택지의 문구다. 번호 열의 커서·입력창은 계속 다른 입력으로 보고 거부한다.

Claude Code 2.1.281·2.1.283의 실제 권한 화면을 격리 PTY에서 렌더링해 기준으로 삼는다. `Yes, and don’t ask again for: <명령 접두어>`, `Yes, allow reading from …`, 권한 모드 전환(`switch to auto mode`, `switch to accept edits (…) for this session`)은 이후 요청의 권한 범위만 바꾸므로 반복 허가로 분류하고 1번 `Yes`를 고른다. 하단의 `Esc to cancel · Tab to amend`를 완전한 키 안내로 인정한다. 브랜치·환경 등 다른 작업으로의 전환은 직접 확인으로 남긴다.

하단 키 안내 전체와 그 뒤에 다른 출력이 없음을 요구하는 규칙은 유지한다. 따라서 하단 안내가 없는 Claude WebFetch 권한 화면은 화면 방식으로 처리하지 않는다. 입력 직전의 전체 화면 비교(Terminal JXA·VS Code 확장)는 바꾸지 않는다. 같은 판별기를 쓰는 Claude 단일 질문 훅도 같은 선택지 문구를 반복 허가로 처리한다. 표시 문구·배치 변화가 없으므로 UI는 바꾸지 않는다.

완료 기준: 승인 내역의 실제 Codex 화면을 여러 폭으로 다시 줄바꿈해도 모두 감지하고, 선택 이동·뒤따르는 입력창·번호 열의 커서가 있는 화면에는 입력하지 않는다. 실제 Claude 2.1.28x 렌더링 화면은 1번으로 감지하고, 선택 이동·하단 안내 뒤 출력·다른 작업 선택지는 직접 확인으로 남긴다.

### 앱을 드래그하는 DMG · 0.2.26

사용자의 DMG 배포 요청에 따라 기본 설치를 Finder의 앱 복사로 변경한다. DMG에는 AutoApprove.app, Applications 바로가기, 설치 안내만 둔다. 기존 macOS 아이콘·파일 복사·교체 확인·권한 요청을 그대로 사용하며 별도 설치 프로그램이나 터미널 명령을 요구하지 않는다. 흐름은 기존 앱 종료 → DMG 열기 → 앱을 Applications로 드래그 → 설치된 앱 열기다. 공증되지 않은 앱의 최초 실행 승인은 시스템 설정에서 사용자가 결정하며, DMG 전환이나 드래그 복사가 quarantine을 자동 제거한다고 안내하지 않는다.

완료 기준은 DMG 내부 앱의 서명·실행 파일 일치, Applications 링크, 내장 PKG/실행 스크립트 없음, 복사본의 서명 유지다. 실제 Finder에서 세 항목을 확인하며 사용자 앱을 교체하지 않는 격리 복사 검증과 최초 보안 경고 이후 실행 검증을 구분한다. 시스템 파일 UI를 사용하므로 새 레이아웃·색상·타이포그래피를 도입하지 않는다.
