...
긍정 응답에는 Allow·Allow once도 포함한다. Deny·Don't allow는 부정 응답이며 Always allow·세션 내 허용만 있는 선택지는 자동으로 고르지 않는다. 여러 긍정 선택지가 있으면 유일한 이번 요청 허용을 선택하고 원래 라벨을 표시·기록·전송한다.
